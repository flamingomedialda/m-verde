...
import { pointsForGrams, formatWeight, type Profile } from "@/lib/types";
...
Saldo actual: ${citizen.points + estPoints} pontos`}
            </pre>
          </div>
    ...
    <MobileShell withNav={false}>
        ...
        <div className="px-5 pb-32">
          ...
          <div className="bg-card rounded-3xl p-6 shadow-card">
            <div className="flex items-center justify-between">
              <button onClick={() => setWeightG(Math.max(0, weightG - 100))} className="h-14 w-14 rounded-2xl bg-muted flex items-center justify-center tap-scale"><Minus className="h-6 w-6" /></button>
              ...
              <button onClick={() => setWeightG(weightG + 100)} className="h-14 w-14 rounded-2xl bg-muted flex items-center justify-center tap-scale"><Plus className="h-6 w-6" /></button>
            </div>
            ...
            <div className="mt-4 text-center text-sm text-muted-foreground">Equivalente a <b className="text-primary">{estPoints} pontos</b></div>
          </div>
        </div>
      )}
      ...
      <div className="fixed bottom-0 inset-x-0 bg-background/95 backdrop-blur border-t border-border p-4 safe-bottom">
        <div className="max-w-md mx-auto">
          <Button size="lg" disabled={busy || (step === 3 && materials.length === 0) || (step === 4 && weightG === 0)}
            onClick={() => {
