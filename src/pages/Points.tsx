...
import { getCurrentUser, store, type Reward, type User } from "@/lib/mockData";
import { Wallet } from "lucide-react";

export default function Points() {
  const [user, setUser] = useState<User | null>(null);
  const [rewards, setRewards] = useState<Reward[]>([]);
  useEffect(() => { setUser(getCurrentUser()); setRewards(store.get().rewards); }, []);
      ...
      <div className="px-5">
        <div className="rounded-3xl p-5 gradient-green text-white shadow-soft flex items-center gap-4">
          ...
          <div>
            ...
            <div className="text-3xl font-extrabold leading-none mt-0.5">{user?.points ?? 0} pts</div>
          </div>
        </div>
      </div>
          ...
          return (
            <Link key={r.id} to={`/points/${r.id}`} className="block tap-scale">
              <article className="bg-card rounded-3xl p-4 shadow-card h-full flex flex-col">
                <div className="h-20 rounded-2xl bg-accent flex items-center justify-center text-4xl mb-3">
                  {r.icon}
                </div>
                ...
                <div className={`mt-auto pt-3 text-sm font-bold ${canRedeem ? "text-primary" : "text-muted-foreground"}`}>
                  {r.points} pts
                </div>
              </article>
            </Link>
