...
export default function Report() {
  ...
  const detectLocation = () => {
    if (!("geolocation" in navigator)) return setArea("Maputo · KaMpfumo");
    navigator.geolocation.getCurrentPosition(
      () => setArea("Maputo · KaMpfumo (GPS)"),
      ...
      {step === 4 && (
        <div className="px-5 pb-32">
          ...
          <div className="bg-card rounded-3xl shadow-card divide-y divide-border">
            ...
            <Row label="Foto" value={photo ? "Anexada" : "Sem foto"} icon={photo ? <ImageIcon className="h-4 w-4" /> : undefined} />
      ...
      {step < 5 && (
        <div className="fixed bottom-0 inset-x-0 bg-background/95 backdrop-blur border-t border-border p-4 safe-bottom">
          <div className="max-w-md mx-auto">
            ...
            >
              {step === 4 ? "Submeter reporte" : "Continuar"}
            </Button>
          </div>
        </div>
      )}
